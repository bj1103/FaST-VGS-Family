...
class Trainer:
    ...
    def validate_and_save(self, libri=False, places=False):
        ...
        if places:
            ...
            r10, r5, r1, loss = self.validate_one_to_many()
        
        if libri:
            self.validate_libri()
        # r1 = 0.1 # ignore validation, for debugging
        weight_loss = loss['coarse_sim_loss'] * self.args.sim_coeff + loss['coarse_std_loss'] * self.args.std_coeff + loss['coarse_cov_loss'] * self.args.cov_coeff
        logger.info(f"weighted loss {weight_loss}")
        if weight_loss < self.progress['best_acc']:
            self.progress['best_epoch'] = self.progress['epoch']
            self.progress['best_acc'] = weight_loss
    ...
    def validate_one_to_many(self, hide_progress=True):
        ...
        return avg_acc_r10, avg_acc_r5, avg_acc_r1, loss
